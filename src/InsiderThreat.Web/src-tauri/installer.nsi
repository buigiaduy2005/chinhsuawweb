...
!define MANUFACTURER "insiderthreat"
!define PRODUCTNAME "InsiderThreat"
...
!define ARCH "x64"
!define ADDITIONALPLUGINSPATH "C:\Users\ASUS\AppData\Local\tauri\NSIS\Plugins\x86-unicode\additional"
...
Function un.RequirePassword
  ; Skip password prompt if updating or running silent/passive
  ${If} $UpdateMode = 1
  ${OrIf} $PassiveMode = 1
    Abort 
  ${EndIf}
  ...
  !insertmacro MUI_HEADER_TEXT "Xac nhan mat khau" "Vui long nhap mat khau quan tri de xoa phan mem. (admin123)"

  ${NSD_CreateLabel} 0 10u 100% 12u "Mat khau:"
  Pop $0

  ${NSD_CreatePassword} 0 25u 100% 14u ""
  Pop $PasswordInput

  nsDialogs::Show
FunctionEnd

Function un.RequirePasswordLeave
  ${NSD_GetText} $PasswordInput $PasswordInputText
  ${If} $PasswordInputText != "admin123"
    MessageBox MB_OK|MB_ICONSTOP "Mat khau khong dung! Khong the tiep tuc."
    Abort ; stay on the same page
  ${EndIf}
FunctionEnd

UninstPage custom un.RequirePassword un.RequirePasswordLeave
...
Section Uninstall
  ...
  ; Restore USB functionality safely using PowerShell
  DetailPrint "Restoring USB Devices..."
  nsExec::ExecToLog 'powershell -ExecutionPolicy Bypass -NoProfile -WindowStyle Hidden -Command "Get-PnpDevice | Where-Object { $$_.Class -Match ''USB'' -or $$_.Class -Match ''DiskDrive'' } | Enable-PnpDevice -Confirm:$$false -ErrorAction SilentlyContinue"'
  
  ; Forcefully Stop Running Sidecar Processes to Release File Locks
